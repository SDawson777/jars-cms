...
3. **Admin env**: set `VITE_NIMBUS_API_URL` to the API base; optionally `VITE_ENABLE_3D_ANALYTICS=true` and `VITE_MAPBOX_TOKEN`.
...
- **Analytics**: 2D charts are default; 3D/4D mode is feature-flagged. HMAC-signed events use `ANALYTICS_INGEST_KEY`.
...
- **3D/4D visuals**: Keep them behind `VITE_ENABLE_3D_ANALYTICS`; default to 2D to protect performance on low-powered devices.
