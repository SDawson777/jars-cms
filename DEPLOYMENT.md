...
- **Env**: `VITE_NIMBUS_API_URL`, `VITE_APP_ENV`, optional `VITE_ENABLE_3D_ANALYTICS`, `VITE_MAPBOX_TOKEN`.
- **Notes**: SPA routing relies on Vercel rewrites (configured in `vercel.json`). 3D mode is feature-flagged.
...
- **Env**: `JWT_SECRET`, `CORS_ORIGINS`, `SANITY_PROJECT_ID`, `SANITY_DATASET`, `SANITY_API_TOKEN`, `ANALYTICS_INGEST_KEY`, optional `OPENAI_API_KEY`.
...
- **Mapbox**: `VITE_MAPBOX_TOKEN` enables 3D map visuals in Admin.
