...
content.get("/legal", async (req,res)=>{
  const p = z.object({ type: z.enum(["terms","privacy","accessibility"]), preview:z.string().optional(), secret:z.string().optional()}).parse(req.query);
  const c = isPreview(new URLSearchParams(req.query as any)) ? drafts : published;
...
content.get("/articles", async (req,res)=>{
  const p = z.object({ page:z.coerce.number().min(1).default(1), limit:z.coerce.number().min(1).max(50).default(20), tag:z.string().optional(), preview:z.string().optional(), secret:z.string().optional()}).parse(req.query);
  const c = isPreview(new URLSearchParams(req.query as any)) ? drafts : published;
  ...
  const items = await c.fetch(`${base} | order(publishedAt desc)[${from}...${from+p.limit}]{
...
content.get("/articles/:slug", async (req,res)=>{
  const c = isPreview(new URLSearchParams(req.query as any)) ? drafts : published;
  const item = await c.fetch(`*[_type=="greenhouseArticle" && slug.current==$s][0]{
    "id":_id, title, "slug":slug.current, excerpt, body,
    "cover":{"src":coverImage.asset->url,"alt":coverImage.alt},
    tags, author, publishedAt, featured
  }`, { s:req.params.slug });
...
content.get("/filters", async (_req,res)=>{
  const c = published;
  const cats = await c.fetch(`*[_type=="shopCategory" && active==true] | order(weight asc){name,"slug":slug.current,iconRef,weight}`);
...
content.get("/deals", async (req,res)=>{
  const p = z.object({ storeId:z.string().optional(), limit:z.coerce.number().min(1).max(50).default(20) }).parse(req.query);
  const c = published;
  const f = p.storeId ? `&& $sid in stores[]` : "";
  const items = await c.fetch(`*[_type=="deal" && active==true && now() >= startAt && now() <= endAt ${f}] | order(priority desc)[0...$lim]{
...
content.get("/copy", async (req,res)=>{
  const p = z.object({ context: z.enum(["onboarding","emptyStates","awards","accessibility","dataTransparency"]) }).parse(req.query);
  const c = published;
  const items = await c.fetch(`*[_type=="appCopy" && context==$ctx]{key,text}[0...100]`, { ctx:p.context });
  res.set("Cache-Control","public, max-age=86400, stale-while-revalidate=300");
  res.json(items);
});
