...
type Action = {targetType: string; targetSlugOrKey: string; priorityBoost: number; channel?: string}
type Rule = {
  _id?: string
  name?: string
  description?: string
...
export async function evaluatePersonalization(
  ...
  for (const rule of rules) {
    ...
    const allMatch = conditions.every((cond) => matchCondition(cond, userContext))
    ...
    for (const action of actions) {
      for (const cand of candidates) {
        ...
        if (action.channel && cand.channel && String(action.channel) !== String(cand.channel))
          continue
        // apply boost
        cand.score = (cand.score || 0) + (Number(action.priorityBoost) || 0)
        cand.reasons.push({rule: rule.name || rule._id, boost: action.priorityBoost})
      }
    }
  }
