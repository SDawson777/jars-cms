...
legalRouter.get('/', async (req, res) => {
  const {type} = z.object({type: z.enum(['terms', 'privacy', 'accessibility'])}).parse(req.query)
  ...
  // map to the mobile-friendly shape: { title, body }
  res.json({title: item.title, body: item.body})
})
