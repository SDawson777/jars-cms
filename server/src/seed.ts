...
export async function seedControlPlane() {
  ...
  const tenantSlug = DEMO_TENANT_SLUG || (APP_ENV === 'demo' ? 'demo-operator' : 'preview-operator')
  ...
  await prisma.featureFlag.createMany({
    data: [
      {tenantId: tenant.id, key: 'ai_concierge', valueJson: {enabled: true}},
      {tenantId: tenant.id, key: 'journal_enabled', valueJson: {enabled: true}},
    ],
  })

  console.log(`[seedControlPlane] Seeded tenant ${tenantSlug} for ${APP_ENV} environment`)
}
